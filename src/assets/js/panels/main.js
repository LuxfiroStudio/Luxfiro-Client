...
class Settings {
        ...
        document.querySelector('.accounts-list').addEventListener('click', async e => {
            ...
            try {
                ...
                if (e.target.classList.contains("delete-profile")) {
                    ...
                    const accountListElement = document.querySelector('.accounts-list');
                    ...
                    if (configClient.account_selected === id) {
                        ...
                        const newInstanceSelect = await this.setInstance(allAccounts[0]);
    ...
    async setInstance(auth) {
        ...
        for (const instance of instancesList) {
            if (instance.whitelistActive) {
                const whitelist = instance.whitelist.find(name => name === auth.name);
    ...
    async javaPath() {
        ...
        let javaPath = configClient?.java_config?.java_path || 'Utiliser la version de java libre avec le launcher';
        ...
        document.querySelector(".java-path-reset").addEventListener("click", async () => {
            let configClient = await this.db.readData('configClient');
            javaPathInputTxt.value = 'Utiliser la version de java libre avec le launcher';
    ...
    async launcher() {
        ...
        let intelEnabledMacCheckbox = document.querySelector("#intel-enabled-mac");
        intelEnabledMacCheckbox.checked = configClient?.launcher_config?.intelEnabledMac || false;

        intelEnabledMacCheckbox.addEventListener('change', async () => {
            let configClient = await this.db.readData('configClient');
            configClient.launcher_config.intelEnabledMac = intelEnabledMacCheckbox.checked;
            await this.db.updateData('configClient', configClient);
        });
    }
}
